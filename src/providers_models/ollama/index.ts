 ...
 *  Priority: $OLLAMA_HOST > $CHEAP_OLLAMA_HOST > http://localhost:11434.
...
export function resolveOllamaHost(): string {
	const fromEnv = process.env.OLLAMA_HOST?.trim() || process.env.CHEAP_OLLAMA_HOST?.trim()
 ...
 *  (including the cheap-dev and cheap-experimental slots managed by
 ...
 *
 * Non-Cheap providers already in models.json are preserved.
 */
