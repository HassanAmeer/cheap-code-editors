 ...
 *  - role pool augmentation (reviewer/builder only, never orchestrator, dedup)
...
import type { OllamaModel } from "./index.js"
import {
...
describe("injectOllamaProvider — models.json merge", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "cheap-ollama-test-"))
		...
		writeModelsJson({
			providers: {
				"cheap-dev": {
					...
					baseUrl: "https://llm.cheap.dev/openai/v1",
	...
	it("preserves cheap-dev, cheap-experimental, and custom providers", async () => {
		writeModelsJson({
			providers: {
				"cheap-dev": { models: [{ id: "cheap-1" }] },
				"cheap-experimental": { models: [{ id: "exp-1" }] },
				"my-custom-provider": {
					api: "openai-completions",
		...
		expect(providers["cheap-dev"].models).toEqual([{ id: "cheap-1" }])
		expect(providers["cheap-experimental"].models).toEqual([{ id: "exp-1" }])
	...
	it("is a no-op when probe returns no models", async () => {
		...
		expect(providers["cheap-dev"]).toBeDefined()
	})
	...
	it("swallows thrown errors silently (does not propagate)", async () => {
		writeModelsJson({ providers: { "cheap-dev": { models: [] } } })
...
describe("readOllamaModelsFromConfig + readOllamaModelMetadata", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "cheap-ollama-read-"))
		modelsJsonPath = join(tmpDir, "models.json")
	})
	...
	it("returns [] when the file has no ollama provider", () => {
		writeFileSync(modelsJsonPath, JSON.stringify({ providers: { "cheap-dev": { models: [] } } }), "utf-8")
		expect(readOllamaModelsFromConfig(modelsJsonPath)).toEqual([])
	})
...
describe("augmentModelRolesWithOllama — role pool augmentation", () => {
	...
	it("adds each model to reviewer and builder", () => {
		...
		expect(result.researcher).toContain("ollama/llama3:8b")
		expect(result.researcher).toContain("ollama/qwen2:70b")
	})

	it("never touches orchestrator and planner", () => {
		const result = augmentModelRolesWithOllama(baseRoles, ollamaModels)
	...
	it("keeps the single-string form when a second model is appended to a single-element array", () => {
		...
		expect(Array.isArray(result.reviewer)).toBe(true)
		expect(result.reviewer as string[]).toEqual(["openai/gpt-5-mini", "ollama/llama3:8b"])
	})
...
describe("resolveOllamaHost — environment variable precedence", () => {
	...
	it("falls back to $CHEAP_OLLAMA_HOST when $OLLAMA_HOST is unset", () => {
		process.env.CHEAP_OLLAMA_HOST = "http://cheap-host:11434"
		expect(resolveOllamaHost()).toBe("http://cheap-host:11434")
	})

	it("$OLLAMA_HOST wins over $CHEAP_OLLAMA_HOST", () => {
		process.env.OLLAMA_HOST = "http://primary:11434"
		process.env.CHEAP_OLLAMA_HOST = "http://fallback:11434"
...
describe("readOllamaModelsFromConfig — defensive reads against malformed models.json", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "cheap-ollama-defensive-"))
		modelsJsonPath = join(tmpDir, "models.json")
	})
	...
	it("returns [] when the file contains an unrelated provider but not ollama", () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				providers: {
					"cheap-dev": { models: [{ id: "should-not-leak" }] },
				},
			}),
			"utf-8",
		)
...
describe("pi-mono ModelRegistry integration — persisted ollama block is accepted", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "cheap-ollama-registry-"))
		modelsJsonPath = join(tmpDir, "models.json")
	})
